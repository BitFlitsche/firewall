...
const InfiniteScrollIPTable = memo(({ 
    ips, 
    loading, 
    error, 
    total,
    ...
    if (ips.length === 0 && !loading) {
        return (
            <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: 200 }}>
                <Typography>No IP addresses found</Typography>
            </Box>
        );
    }
                ...
                <TableContainer>
                                        ...
                                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                            <Box
                                                sx={{
                                                    ...
                                                    borderRadius: '50%',
                                                    backgroundColor: 
                                                        ipItem.status === 'allowed' ? '#28a745' :
                                                        ipItem.status === 'denied' ? '#dc3545' :
                                                        ipItem.status === 'whitelisted' ? '#007bff' : '#6c757d'
                                                }}
                                            />
                                            {ipItem.status}
                                        </Box>
                                        ...
                                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                            <Box
                                                sx={{
                                                    ...
                                                    height: 8,
                                                    borderRadius: '50%',
                                                    backgroundColor: ipItem.is_cidr ? '#007bff' : '#28a745'
                                                }}
                                            />
                                            {ipItem.is_cidr ? 'CIDR Block' : 'Single IP'}
                                        </Box>
                                    </TableCell>
                    ...
                    {!hasMore && ips.length > 0 && (
                        <Box sx={{ 
                            ...
                            <Typography variant="body2" color="text.secondary">
                                Showing {ips.length} of {total} entries
                            </Typography>
                        </Box>
                    )}
                </TableContainer>
...
export default InfiniteScrollIPTable; 